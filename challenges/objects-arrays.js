...
tyrannousaurus.sound = function(){
  return 'RAWERSRARARWERSARARARRRR'
}

console.log(tyrannousaurus.sound())
...
const universities =  graduates.map((gradUniversities) => {
  return {'university': gradUniversities.university}
})
...
const contactInfo = graduates.map((studentInfo) => {
  return {'first_name': studentInfo.university, 'email': studentInfo.email}
})
...
*/
const populationTotal = zooAnimals.reduce(function(cb, count){
  const populationTotal = zooAnimals.reduce(function(animals, current){
    return animals + current.population
  }, 0);
